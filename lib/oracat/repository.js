...
import { signGetRequest } from "./http-signatures";
...
export async function listTimeline(request, viewerEmail = "") {
  ...
  if (!client) {
    ...
    return [...localPosts, ...inboxRemotePosts].sort(sortTimelinePosts);
  }
  ...
  return [...localPosts, ...remoteTimelinePosts].sort(sortTimelinePosts);
}

export async function listUserPosts(username, request) {
  const client = serverClient();
  const host = getHostFromRequest(request);

  if (!client) {
    const store = readFallbackStore();
    const account = store.accounts.find((entry) => entry.username === username);
    const posts = store.posts
      .filter((post) => post.author_username === username)
      .map((post) => serializeTimelinePost(post, account, host, ""));
    return { account, posts };
  }

  const { data: account } = await client.from("oracat_accounts").select("*").eq("username", username).maybeSingle();
  if (!account) {
    return { account: null, posts: [] };
  }
  ...
  const serialized = (posts || []).map((post) => serializeTimelinePost(post, account, host, ""));
  return { account, posts: serialized };
}
/*调用listLikedPosts的帖子 */
export async function listLikedPosts(username, request) {
  const client = serverClient();
  const host = getHostFromRequest(request);
  ...
  if (!client) {
    ...
    return store.posts
      ...
      .map((post) => {
        const author = store.accounts.find(
          (entry) => entry.username === post.author_username
        );

        return serializeTimelinePost(
          post,
          author,
          host,
          username
  ...
  return (posts || [])
    .map((post) =>
      serializeTimelinePost(
        post,
        accountMap.get(post.author_username),
        host,
        username
      )
    )
...
/*调用收藏的帖子*/
export async function listBookmarkedPosts(username, request) {
  ...
  const { data: posts, error } = await client
    .from("oracat_posts")
    .select("*")
    .contains("bookmarked_by", [username])
    .order("published_at", {
      ascending:false
    });
...
export async function applyRemoteInteractionToLocalPost({ postId, actorHandle, action }) {
  const client = serverClient();
  if (!client) {
    const store = readFallbackStore();
    ...
    const updated = applyLocalPostMutation(post, actorHandle, action, "");
    return updateFallbackPost(postId, updated);
  }

  const { data: post } = await client.from("oracat_posts").select("*").eq("id", postId).maybeSingle();
  if (!post) {
    return null;
  }

  const updatedPayload = applyLocalPostMutation(post, actorHandle, action, "");
  const { data, error } = await client
    .from("oracat_posts")
  ...
  if (post.author_username) {
    await createRemoteInteractionNotification({
...
async function createRemoteInteractionNotification({ recipientUsername, actorHandle, action }) {
  if (!["like", "boost"].includes(action)) {
    return null;
  }
  const notification = {
    recipient_username: recipientUsername,
    actor_username: actorHandle,
    actor_display_name: actorHandle,
    type: action,
    summary: "",
    read_at: null,
    created_at: new Date().toISOString()
  ...
  const client = serverClient();
  if (!client) {
    return pushFallbackNotification({ id: randomUUID(), ...notification });
  }
  try {
    const { data, error } = await client.from("oracat_notifications").insert(notification).select("*").single();
    if (error) {
      throw error;
    }
    return data;
  } catch (error) {
    console.error("Failed to create remote interaction notification:", error.message);
    return null;
  }
}
...
export async function mutatePostInteraction({ postId, email, displayName, action, content = "" }) {
  ...
  // 真删除：本人的本地帖子，直接从数据库移除，而不是标记为 [deleted]
  if (action === "delete") {
    if (!client) {
      const store = readFallbackStore();
      const localPost = store.posts.find((entry) => entry.id === postId);
      if (localPost && localPost.author_username === viewer.username) {
        store.posts = store.posts.filter((entry) => entry.id !== postId);
        return { id: postId, deleted: true };
      }
      return null;
    }
    const { data: localPost } = await client.from("oracat_posts").select("id,author_username").eq("id", postId).maybeSingle();
    if (localPost && localPost.author_username === viewer.username) {
      const { error } = await client.from("oracat_posts").delete().eq("id", postId);
      if (error) {
        throw error;
      }
      return { id: postId, deleted: true };
  ...
  // 转发：在原帖 boosted_by 上加名字的同时，在转发者的时间线上生成一条新帖子
  if (action === "boost") {
    if (client) {
      const { data: originalPost } = await client.from("oracat_posts").select("*").eq("id", postId).maybeSingle();
      if (originalPost) {
        const updatedPayload = applyLocalPostMutation(originalPost, viewer.username, action, content);
        ...
        const alreadyBoosted = (originalPost.boosted_by || []).includes(viewer.username);
        if (!alreadyBoosted) {
          const boostPost = {
            id: randomUUID(),
            ...
            poll: null,
            link_previews: originalPost.link_previews || [],
            liked_by: [],
            boosted_by: [],
            bookmarked_by: [],
            comments: [],
            boosted_from_id: originalPost.id,
            boosted_from_author: originalPost.author_username,
            published_at: new Date().toISOString()
          };
          await client.from("oracat_posts").insert(boostPost);
        }

        await createNotificationForMutation({ post: originalPost, viewer, action, content, source: "local" });
...
export async function searchRemoteAccount(query, signingAccount = null) {
  const normalized = String(query || "").trim();
  if (!normalized) {
    return null;
  }
  if (normalized.startsWith("http://") || normalized.startsWith("https://")) {
    return importRemoteActor(normalized, signingAccount);
  }
  ...
  return importRemoteActor(selfLink.href, signingAccount);
}
...
export function serializeRemoteActorForClient(actor, localUsername = "") {
  ...
  return {
    ...
    followingState: localUsername ? (actor.following_state || actor.state || "none") : "unknown"
  };
}
...
function serializeTimelinePost(post, author, host, viewerUsername = "", persistedPoll = null) {
  ...
  const poll = persistedPoll || normalizePoll(post.poll, viewerUsername);
 /* 之前序列化层重复转换，导致状态被重置，persistedPoll 已经是最终结果
    不需要再次 normalize，重新 normalize 会丢失 counts*/

  return {
    id: post.id,
    publishedAt: post.published_at, /*修改中间时间戳，让最新的排在最上面*/
    author: {
      name: author?.display_name || post.author_username,
      handle: `@${post.author_username}@${author?.domain || host}`, 
      instance: author?.domain || host,
      badge: (!author?.domain || author?.domain === host) ? "Local" : "Remote",
...
function extractTags(content) {
  ...
  const extracted = keywords.filter((word) =>
...
function sortTimelinePosts(a, b) {
  const timeA = new Date(a.published_at || 0).getTime();/* 用真实的原始时间排序*/
  const timeB = new Date(b.published_at || 0).getTime();
  return timeB - timeA; // 越新越靠前
}

export async function unfollowRemoteActor({ localUsername, remoteActorUrl }) {
  const client = serverClient();
  if (!client) {
    return null;
  }
...
export async function importRemoteActor(actorUrlValue, signingAccount = null) {
  const headers = { Accept: "application/activity+json" };

  if (signingAccount?.private_key_pem) {
    const { signGetRequest } = await import("./http-signatures");
    const { actorUrl } = await import("./config");
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "https://mintcat.vercel.app";
    const keyId = `${actorUrl(baseUrl, signingAccount.username)}#main-key`;
    Object.assign(headers, signGetRequest({
      privateKeyPem: signingAccount.private_key_pem,
      url: actorUrlValue,
      keyId
    }));
  }
  ...
  return {
    ...
    avatar_url: payload.icon?.url || null,
    summary: payload.summary ? String(payload.summary).replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim() : "",
...
export async function getFollowerProfiles(localUsername) {
  const followers = await getFollowers(localUsername);
  const client = serverClient();
  ...
  const actorUrls = followers.map((entry) => entry.remote_actor_url);
...
async function getPollVoteMap(client, localPosts, remotePosts, viewerUsername = "") {
  ...
  if (localIds.length) {
    filters.push(
  `and(post_source.eq.local,post_id.in.(${localIds.map(id => `"${id}"`).join(",")}))`
);
  }
  if (remoteIds.length) {
    filters.push(
  `and(post_source.eq.remote,post_id.in.(${remoteIds.map(id => `"${id}"`).join(",")}))`
);
  }
...
async function persistPollVote(client, { postSource, postId, voterUsername, optionText }) {
  ...
  );/* 7.28,修改了supabase 的字段，使它与上面这一行的相同，避免supabase约束冲突报错*/
...
export async function createFollowNotification({ recipientUsername, actorUsername, actorDisplayName }) {
  ...
  const notification = {
    ...
    actor_display_name: actorDisplayName || actorUsername,
    type: "follow",
    summary: "",
    read_at: null,
    created_at: new Date().toISOString()
...
/* 之前不能投靠，是数据库字段与约束问题，以及Aggregation 和 Serializer 两层之间的数据契约不一致。
 社交平台的常见问题，同一个对象会经过多个转换层：数据库 → repository → serializer → API → frontend*/

 /* 这次修复，说明mintcat架构已经接近真实社交系统的结构，
 Post Storage
     ↓
Interaction Storage
     ↓
...
/* 把错误分类
42xxx → PostgreSQL 结构问题
4xx → API 请求问题
5xx → 后端逻辑问题
build error → 前端/依赖问题*/

/* 已经下载supabse,但是还未从supabase上拉取数据库字段约束等，到本地项目*/