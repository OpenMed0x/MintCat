...
export async function listTimeline(request, viewerEmail = "") {
  ...
  // 帖子的published_at早于36h前，不会被查询
  const cutoffTimestamp = new Date(Date.now() - 36 * 60 * 60 * 1000).toISOString();
  const [{ data: accounts }, { data: posts }, { data: remotePosts }] = await Promise.all([
    client.from("oracat_accounts").select("*"),
    client.from("oracat_posts").select("*").gte("published_at", cutoffTimestamp).order("published_at", { ascending: false }).limit(40),
    client.from("oracat_remote_posts").select("*").gte("published_at", cutoffTimestamp).order("published_at", { ascending: false }).limit(20)
  ]);
...
function sortTimelinePosts(a, b) {
  const timeA = new Date(a.published_at || a.minutesAgo * 60000 || 0).getTime();
  const timeB = new Date(b.published_at || b.minutesAgo * 60000 || 0).getTime();
