import { followAcceptActivity, createActivity, followActivity,undoFollowActivity,likeActivity, announceActivity } from "./activitypub";
...
export async function deliverUnfollow({ localAccount, remoteActor, originalFollowId, baseUrl }) {
  if (!remoteActor.inbox_url || !localAccount.private_key_pem) {
    throw new Error("Remote actor inbox or local key is missing.");
  }
  await deliverActivity({
    activity: undoFollowActivity(baseUrl, localAccount, remoteActor, originalFollowId),
...
export async function deliverLike({ localAccount, remoteObjectUrl, remoteInboxUrl, baseUrl }) {
  await deliverActivity({
    activity: likeActivity(baseUrl, localAccount, remoteObjectUrl),
    inboxUrl: remoteInboxUrl,
    privateKeyPem: localAccount.private_key_pem,
    keyId: `${actorUrl(baseUrl, localAccount.username)}#main-key`
  });
}

export async function deliverAnnounce({ localAccount, remoteObjectUrl, remoteInboxUrl, baseUrl }) {
  await deliverActivity({
    activity: announceActivity(baseUrl, localAccount, remoteObjectUrl),
    inboxUrl: remoteInboxUrl,
    privateKeyPem: localAccount.private_key_pem,
    keyId: `${actorUrl(baseUrl, localAccount.username)}#main-key`
  });
