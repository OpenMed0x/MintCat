...
export function buildSigningStringGet({ path, host, date }) {
  return [`(request-target): get ${path}`, `host: ${host}`, `date: ${date}`].join("\n");
}

export function signGetRequest({ privateKeyPem, url, keyId }) {
  const { pathname, search, host } = new URL(url);
  const date = new Date().toUTCString();
  const signingString = buildSigningStringGet({
    path: `${pathname}${search}`,
    host,
    date
  });
